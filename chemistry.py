...
# ================================================================================
# [2] 연세대 화학과 리스트(List) 페이지 크롤링 엔진 (NEW)
# ================================================================================
def get_chemistry_links(list_url):
    """사용자 요청 반영: 상단 고정 공지(nxb-list-table__notice)를 제외한 일반 tr의 링크를 수집합니다."""
    links = []
    try:
        headers = {
        ...
        for row in rows:
            # ★ 핵심 필터: 고정 공지 클래스(nxb-list-table__notice) 제외
            row_classes = row.get('class', [])
            if row_classes and 'nxb-list-table__notice' in row_classes:
                continue
            ...
            # 2. 제목 추출
            # 제목에 섞여있는 [새글] 아이콘 텍스트 등 불필요한 공백/요소를 제거하고 순수 텍스트만 추출
            title = a_tag.get_text(strip=True)
            if not title:
                continue
            
            # 3. 글 번호 추출 (보통 첫 번째 td에 위치)
            num_text = "일반"
            tds = row.find_all('td')
            if tds:
...
# ================================================================================
# [3] 연세대 화학과 상세 페이지 크롤링 엔진
# ================================================================================
def scrape_chemistry_detail(url):
    try:
        ...
        # 1. 제목 추출 (<h3 class="nxb-view__header-title text-xxlarge">)
        title = "제목 없음"
        title_tag = soup.find('h3', class_=lambda c: c and 'nxb-view__header-title' in c)
        if title_tag:
        ...
        # 3. 본문 및 4. 이미지 추출 (<div class="editor-contents">)
        content_html = ""
        images = []
        ...
        # 5. 첨부파일 추출 (<p class="nxb-view__files-text text-small">)
        attachments = []
        file_p_tags = soup.find_all('p', class_=lambda c: c and 'nxb-view__files-text' in c)
...
# [4] UI 화면
# ================================================================================
st.set_page_config(page_title="연세 화학과 공지 크롤러", layout="wide")
st.title("🧪 연세대학교 화학과 공지사항 추출기")
st.markdown("**목록 페이지**를 입력하면 상단 고정 공지(`nxb-list-table__notice`)를 제외하고 일반 게시물을 자동으로 긁어옵니다.")
...
# 화학과 공지사항 URL
list_url_input = st.text_input("🔗 연세 화학과 **목록(List)** URL", value="https://chemyonsei.kr/board/notice")
...
if st.button("최신 공지사항 긁어오기", type="primary"):
    if not list_url_input:
        ...
        # 1. 목록에서 링크 추출
        with st.spinner("화학과 게시물 목록 스캔 중... (고정 공지 제외)"):
            post_links = get_chemistry_links(list_url_input)
        ...
        if not post_links:
            ...
            for idx, item in enumerate(post_links):
                with st.expander(f"[{item['no']}] {item['title']}", expanded=True):
                    title, date, content, images, attachments = scrape_chemistry_detail(item['url'])
