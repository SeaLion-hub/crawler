...
# ================================================================================
# [2] 연세대 물리학과 리스트(List) 페이지 크롤링 엔진 (NEW)
# ================================================================================
def get_physics_links(list_url):
    """사용자 요청 반영: 상단 고정 공지(bl_notice)를 제외한 일반 tr의 링크를 수집합니다."""
    links = []
    try:
        headers = {
        ...
        for row in rows:
            # ★ 핵심 필터: 고정 공지 클래스(bl_notice) 제외
            row_classes = row.get('class', [])
            if row_classes and 'bl_notice' in row_classes:
                continue
...
# ================================================================================
# [3] 연세대 물리학과 상세 페이지 크롤링 엔진
# ================================================================================
def scrape_physics_detail(url):
    try:
        ...
        # 1. 제목 추출 (<div class="bw_header"> 내의 <h3>)
        title = "제목 없음"
        ...
        # 2. 날짜 추출 ("작성일" 타이틀을 찾아서 그 옆의 값을 추출)
        date = "날짜 없음"
        if header_div:
            ...
            if info_ul:
                for li in info_ul.find_all('li'):
                    ...
                    if dt_span and '작성일' in dt_span.get_text():
                        ...
                        if dd_span:
                            date_str = dd_span.get_text(strip=True)
                            date = normalize_date(date_str)
                            break

        # 3. 본문 및 4. 이미지 추출 (<div class="bw_contents editor_contents">)
        content_html = ""
        images = []
        
        # 클래스에 bw_contents가 포함된 div 타격
        content_div = soup.find('div', class_=lambda c: c and 'bw_contents' in c)
        ...
        # 5. 첨부파일 추출 (<div class="bw_files"> 내의 <a> 태그)
        attachments = []
...
# [4] UI 화면
# ================================================================================
st.set_page_config(page_title="연세 물리학과 공지 크롤러", layout="wide")
st.title("⚛️ 연세대학교 물리학과 공지사항 추출기")
st.markdown("**목록 페이지**를 입력하면 상단 고정 공지(`bl_notice`)를 제외하고 모든 일반 게시물을 자동으로 긁어옵니다.")
...
# 물리학과 공지사항 URL
list_url_input = st.text_input("🔗 연세 물리학과 **목록(List)** URL", value="https://physicsyonsei.kr/notice/board")
...
if st.button("최신 공지사항 긁어오기", type="primary"):
    if not list_url_input:
        ...
        # 1. 목록에서 링크 추출
        with st.spinner("물리학과 게시물 목록 스캔 중... (고정 공지 제외)"):
            post_links = get_physics_links(list_url_input)
        ...
        if not post_links:
            ...
            for idx, item in enumerate(post_links):
                with st.expander(f"[{item['no']}] {item['title']}", expanded=True):
                    title, date, content, images, attachments = scrape_physics_detail(item['url'])
